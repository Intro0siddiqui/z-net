...
impl SmartMiddleware {
    ...
    pub fn decompress(&self, data: &[u8], encoding: &str) -> Result<Vec<u8>, std::io::Error> {
        ...
        for enc in encodings.into_iter().rev() {
            ...
            match enc.as_str() {
                "gzip" | "x-gzip" => {
                    let mut decoder = GzDecoder::new(current_data.as_slice());
                    decoder.read_to_end(&mut decompressed)?;
                    current_data = decompressed;
                }
                "br" | "brotli" => {
                    let mut decoder = Decompressor::new(current_data.as_slice(), 4096); // 4KB buffer
                    decoder.read_to_end(&mut decompressed)?;
                    current_data = decompressed;
                }
                _ => {
                    // Unknown token, fallback to raw bytes
                    break;
                }
            }
        }
    ...
    pub fn handle_redirect(&self, status_code: u16, location: Option<&str>, base_url: &Url, redirect_count: &mut u32) -> Option<Url> {
        if (300..400).contains(&status_code) {
            if *redirect_count < self.max_redirects {
                if let Some(loc) = location {
                    if let Ok(new_url) = Url::parse(loc).or_else(|_| base_url.join(loc)) {
                        *redirect_count += 1;
                        return Some(new_url);
                    }
                }
...
impl FetchEngine {
    ...
    pub fn fetch(&mut self, mut url: Url) {
        ...
        loop {
            ...
            if let Some(new_url) = self.middleware.handle_redirect(status_code, location_header, &url, &mut redirect_count) {
                url = new_url;
                continue;
            }
...
mod tests {
    ...
    fn test_handle_redirect() {
        ...
        let new_url = middleware.handle_redirect(301, Some("https://example.com/new"), &base_url, &mut redirect_count);
        ...
        // Exceed redirects
        redirect_count = 5;
        let no_url = middleware.handle_redirect(301, Some("https://example.com/new2"), &base_url, &mut redirect_count);
    ...
    fn test_fetch_engine_mock_network() {
        let mut engine = FetchEngine::new(30000);
